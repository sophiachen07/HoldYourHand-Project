# 🖐️ Hold Your Hand (Aura) - BFRB 智慧穿戴與習慣逆轉輔助系統
...
[功能特色](#-核心功能特色) • [系統架構](#-系統架構與技術棧) • [BLE 通訊協議](#-ble-藍牙通訊協議) • [快速開始](#-快速上手指南) • [打包發布](#-android-apk-打包指南)
...
**Hold Your Hand (Aura)** 是一套結合**智慧穿戴手環硬體**與**行動端 App** 的數位健康輔助系統。

身體專注重複行為（**BFRB**, Body-Focused Repetitive Behaviors，如**拔毛症 Trichotillomania**、**咬指甲 Onychophagia**、**摳皮症 Excoriation** 等）往往發生在無意識狀態、壓力焦慮或無聊缺乏刺激的情境中。
...
- **多維度感測指標監控**：
  ...
  - 🔄 **空間角速度 (Angular Velocity)**：3 軸空間向量模長 $\sqrt{g_x^2 + g_y^2 + g_z^2}$，即時追蹤舉手與擺動動態。
  - ❤️ **心率 (Heart Rate BPM)** 與 💧 **血氧 (SpO2 %)**：監控生理緊張度與自律神經狀態。
...
- **高品質環境白噪音**：內建「正念靜心 (mindsound)」、「舒緩海浪 (ocean)」、「自然雨聲 (rain)」，支援無縫循環播放與獨立音量調節。
...
## 🛠️ 系統架構與技術棧 (Tech Stack & Architecture)
...
| 領域 | 使用技術 |
| :--- | :--- |
| **核心框架** | [React Native](https://reactnative.dev/) 0.86.3, [Expo](https://expo.dev/) SDK 57, [TypeScript](https://www.typescriptlang.org/) |
| **路由與導航** | [Expo Router](https://docs.expo.dev/router/introduction/) (File-based Routing, Typed Routes) |
| **藍牙通訊** | [react-native-ble-plx](https://github.com/dotintent/react-native-ble-plx) |
| **觸覺與多媒體**| [expo-haptics](https://docs.expo.dev/versions/latest/sdk/haptics/), [expo-audio](https://docs.expo.dev/versions/latest/sdk/audio/) |
| **圖表與動畫** | [react-native-svg](https://github.com/software-mansion/react-native-svg), [react-native-reanimated](https://docs.swmansion.com/react-native-reanimated/), [expo-linear-gradient](https://docs.expo.dev/versions/latest/sdk/linear-gradient/) |
| **本機資料儲存** | [@react-native-async-storage/async-storage](https://react-native-async-storage.github.io/async-storage/) |
| **圖標庫** | [lucide-react-native](https://lucide.dev/) |
| **開發與模擬伺服器** | Node.js, [ws](https://github.com/websockets/ws) WebSocket Server, HTML5 Canvas Simulator |
...
| 欄位順序 | 欄位名稱 | 單位 / 型態 | 說明 |
| :---: | :--- | :--- | :--- |
| 1 | `distance` | `int` (mm) | 距離數值，`>= 9999` 代表超出感應範圍 |
| 2 | `angularVelocity` | `float` (deg/s) | 空間角速度向量模長 $\sqrt{gx^2+gy^2+gz^2}$ |
| 3 | `heartRate` | `float` (BPM) | 即時心率 |
| 4 | `spo2` | `int` (%) | 即時血氧飽和度 |
| 5 | `motorStatus` | `int` (0 或 1) | `1` 為馬達震動中，`0` 為待機/冷卻 |

> **封包範例**：原始字串 `45,32.4,84.2,98,1` $\rightarrow$ Base64 編碼 `NDUsMzIuNCw4NC4yLDk4LDE=`
...
- 手機安裝 [Expo Go](https://expo.dev/go) (適用於介面與模擬測試) 或 **Development Build / 實機 APK** (適用於原生藍牙 BLE 測試)
...
# 3. 建立 Commit 訊息
git commit -m "docs: update comprehensive project README with architecture, BLE protocol, and build instructions"
...
本專案採用 [MIT License](LICENSE) 開源授權。歡迎自由學習、改進與交流！

<div align="center">
Made with ❤️ for BFRB Awareness & Habit Reversal
</div>
