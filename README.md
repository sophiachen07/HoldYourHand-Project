...
[功能特色](#核心功能特色-key-features) • [系統架構](#系統架構與技術選型-tech-stack--architecture) • [BLE 通訊協議](#ble-藍牙通訊協議-ble-hardware-protocol) • [專案目錄](#專案目錄結構-project-structure) • [快速開始](#快速上手指南-getting-started) • [APK 打包](#android-apk-打包指南-build-apk)
...
- **多維度感測指標監控**：
  ...
  - **心率 (Heart Rate BPM)** 與 **血氧 (SpO2 %)**：監控生理緊張度與自律神經狀態。
...
## 系統架構與技術選型 (Tech Stack & Architecture)
...
| 領域 | 使用技術 |
| :--- | :--- |
| **核心框架** | [React Native](https://reactnative.dev/) 0.86.3, [Expo](https://expo.dev/) SDK 57, [TypeScript](https://www.typescriptlang.org/) 5.0+ |
...
```text
HoldYourHand-Project/
├── aura-mobile/            # 行動端應用程式 (React Native / Expo)
...
│   ├── contexts/           # 核心全域狀態管理 (AlertContext, SensorContext, UserContext)
│   ├── hooks/              # 自訂 React Hooks
│   ├── services/           # 底層通訊服務 (bleService.ts)
...
│   ├── package.json        # 伺服器依賴設定
│   └── server.js           # WebSocket 廣播與封包轉發邏輯
└── aura-simulator/         # 網頁端感測器控制面板與模擬器
    ├── dev-tools.html      # 開發者除錯與封包測試工具
    └── index.html          # 視覺化感測滑桿與動作模擬介面
```
